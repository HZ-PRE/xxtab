...
- **检查更新**：Windows/macOS 启动时自动检查一次 GitHub Releases，有新版本时弹窗询问下载；菜单也可手动检查。下载对应平台包并校验 SHA256，确认后断开连接并打开安装包，保留配置。Linux 启动 `run` / `relay` 时自动检查，桌面环境可通过已安装的 zenity/kdialog 弹窗，纯终端输出提示；使用 `xxtab update download` 下载。详见 [程序更新](docs/updating.md)。
